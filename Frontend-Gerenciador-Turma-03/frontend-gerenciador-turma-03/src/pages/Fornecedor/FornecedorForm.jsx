...
const FornecedorForm = () => {
  ...
  const handleSubmit = (event) => {
    ...
    const fornecedorData = {
      ...fornecedor,
      //remove os macadores do CNPJ par virar um somente numeros
      cnpj: fornecedor.cnpj.replace(/[^\d]/g, ""),
    };

    //Determinar se a requisição será PUT ou POST
    const request = id
    ...
    request
      ...
      .catch((error) => {
        if (error.response && error.response.status === 500) {
          setMensagensErro([
            "Error no sistema, entre em contato com o suporte",
          ]);
    ...
    <div className="form-container">
      ...
      >
        <div className="modalContent">
          ...
          <br />
          <button onClick={fecharModalErro} className="btn-seconday">Fechar</button>
        </div>
      </Modal>
    </div>
  );
